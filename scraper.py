...
def get_clubs(soup):
    ...
    for a single club.
    """
    elts = soup.findAll('div')
    elts_2 = elts[3].findAll('div')
    #print(type(elts_2))
    club_soups = []
    for element in elts[3].findAll('div'):
        club_soups.append(element)
    club = []
    for x in range(0, len(club_soups), 2):
        club.append(club_soups[x])
    return club # TODO: Implement this function
...
def get_club_tags(club):
    ...
    if len(elts) < 1:
        return ''
    labels = []
    for x in range(len(elts)):
        labels.append(elts[x].text)
    return labels # TODO: Implement this function
